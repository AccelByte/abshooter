...
#define MICROPROFILE_EVENT_BUFFER_SIZE (1 << 10) //must be power of 2
#define MICROPROFILE_EVENT_BUFFER_SAMPLING 10
...
#define MP_STRCASESTR StrStrIA
#define MP_THREAD_LOCAL __declspec(thread)
#define MP_NOINLINE __declspec(noinline)

void MicroprofileDebugLog(const char* msg, ...)
{
    char s[1025];
    va_list args;
    memset(s, 0, 1025 * sizeof(s[0]));
    va_start(args, msg);
    vsprintf(s, msg, args);
    va_end(args);
    s[1024] = 0;
    //OutputDebugStringA(s);
    //UE4_LOG(LogTemp, Log, FString(ANSI_TO_TCHAR(s)));
    //FString message = FString(ANSI_TO_TCHAR(s));
...
#if MICROPROFILE_JUSTICE_SERVER
struct MicroProfileJusticeBuffer
{
    float Data[MICROPROFILE_EVENT_BUFFER_SIZE];
    float DynamicFunc[1][MICROPROFILE_EVENT_BUFFER_SIZE];
    std::atomic<uint32_t> nGet;
    std::atomic<uint32_t> nPut;
};

//void MicroProfileJusticeFormatNowTime(char* pFormattedTime);
void MicroProfileJusticeCreateUuid(char* pStrUuid);
void MicroProfileJusticeEventsStart();
void MicroProfileJusticeEventsStop();
void MicroProfileJusticeBufferReset();
void MicroProfileJusticeEventsUpdate();
bool MicroProfileJusticeBufferPut(float item, float difunc);
//bool MicroProfileJusticeBufferGet(float* pItem);
void* MicroProfileJusticeSenderThread(void *);
#endif
...
{
    char* pBufferAllocation = nullptr;
...
#elif MICROPROFILE_GPU_TIMERS_D3D12 == 1
#include <d3d12.h>
...
// Mod by AccelByte
#if MICROPROFILE_JUSTICE_SERVER
    bool                            bJusticeEventsEnabled;
    MicroProfileJusticeBuffer       JusticeBuffer;
    MpSocket                        JusticeEventsSocket;
    MicroProfileThread              JusticeEventsThread;
    bool                            bIsJusticeEventsSending;
    int								BufferSampling;
	int								nJusticeBufferDelay;
#endif
...
void MicroProfileSetWebServerEnable(bool Enable)
{
    S.WebServerEnable = Enable;
}
...
#if MICROPROFILE_JUSTICE_SERVER
        S.bJusticeEventsEnabled = true;
        S.bIsJusticeEventsSending = false;
        S.JusticeEventsThread = nullptr;;
		S.BufferSampling = MICROPROFILE_EVENT_BUFFER_SAMPLING;
		S.nJusticeBufferDelay = MICROPROFILE_GPU_FRAME_DELAY + 1;

		if (!S.bIsJusticeEventsSending && S.bJusticeEventsEnabled)
		{
			MicroProfileJusticeEventsStart();
		}
#endif
    }
...
void MicroProfileShutdown()
{
    ...
    MicroProfileJoinContextSwitchTrace();
    {
        ...
        free(S.WSBuf.pBufferAllocation);
    }
}
...
void MicroProfileFlip_CB(void* pContext, MicroProfileOnFreeze FreezeCB)
{
    ...
    if(S.nWebServerDataSent == (uint64_t)-1 && S.WebServerEnable)
    {
...
#if MICROPROFILE_JUSTICE_SERVER
		if (S.nJusticeBufferDelay > 0)
		{
			S.nJusticeBufferDelay--;
		}
		else
		{
			MicroProfileJusticeEventsUpdate();
		}
#endif
	}
    S.nAggregateClear = 0;
...
void MicroProfilePrintf2(std::function< void(void*, size_t, const char*)> CB, void* Handle, const char* pFmt, ...)
{
	char buffer[32 * 1024];
	va_list args;
	va_start(args, pFmt);
...
void MicroProfileDumpHtml(MicroProfileWriteCallback CB, void* Handle, uint64_t nMaxFrames, const char* pHost, uint64_t nStartFrameId)
{
...
void MicroProfileDumpHtml2(std::function< void(void*, size_t , const char*)> CB, void* Handle, uint64_t nMaxFrames, const char* pHost, uint64_t nStartFrameId)
{
	//stall pushing of timers
	uint64_t nActiveGroup[MICROPROFILE_MAX_GROUP_INTS];
	memcpy(nActiveGroup, S.nActiveGroups, sizeof(S.nActiveGroups));
	...
	float fToMsGPU = MicroProfileTickToMsMultiplier(MicroProfileTicksPerSecondGpu());
	float fAggregateMs = fToMsCPU * (nTicks - S.nAggregateFlipTick);
	MicroProfilePrintf2(CB, Handle, "S.DumpHost = '%s';\n", pHost ? pHost : "");
	time_t CaptureTime;
	time(&CaptureTime);
	MicroProfilePrintf2(CB, Handle, "S.DumpUtcCaptureTime = %ld;\n", CaptureTime);
	MicroProfilePrintf2(CB, Handle, "S.AggregateInfo = {'Frames':%d, 'Time':%f};\n", S.nAggregateFrames, fAggregateMs);

	//categories
	MicroProfilePrintf2(CB, Handle, "S.CategoryInfo = Array(%d);\n", S.nCategoryCount);
	for (uint32_t i = 0; i < S.nCategoryCount; ++i)
	{
		MicroProfilePrintf2(CB, Handle, "S.CategoryInfo[%d] = \"%s\";\n", i, S.CategoryInfo[i].pName);
	}

	//groups
	MicroProfilePrintf2(CB, Handle, "S.GroupInfo = Array(%d);\n\n", S.nGroupCount);
	uint32_t nAggregateFrames = S.nAggregateFrames ? S.nAggregateFrames : 1;
	float fRcpAggregateFrames = 1.f / nAggregateFrames;
	(void)fRcpAggregateFrames;
	char ColorString[32];
	for (uint32_t i = 0; i < S.nGroupCount; ++i)
	{
		...
		MicroProfilePrintf2(CB, Handle, "S.GroupInfo[%d] = MakeGroup(%d, \"%s\", %d, %d, %d, %f, %f, %f, '%s');\n",
			S.GroupInfo[i].nGroupIndex,
			S.GroupInfo[i].nGroupIndex,
			S.GroupInfo[i].pName,
			S.GroupInfo[i].nCategory,
	...
	MicroProfilePrintf2(CB, Handle, "\nS.TimerInfo = Array(%d);\n\n", S.nTotalTimers);
	for (uint32_t i = 0; i < S.nTotalTimers; ++i)
	{
		uint32_t nIdx = i * 2;
		MP_ASSERT(i == S.TimerInfo[i].nTimerIndex);
		MicroProfilePrintf2(CB, Handle, "S.Meta%d = [];\n", i);
		MicroProfilePrintf2(CB, Handle, "S.MetaAvg%d = [];\n", i);
		MicroProfilePrintf2(CB, Handle, "S.MetaMax%d = [];\n", i);

		uint32_t nColor = S.TimerInfo[i].nColor;
		uint32_t nColorDark = (nColor >> 1) & ~0x80808080;
		MicroProfilePrintf2(CB, Handle, "S.TimerInfo[%d] = MakeTimer(%d, \"%s\", %d, '#%02x%02x%02x','#%02x%02x%02x', %f, %f, %f, %f, %f, %f, %d, %f, S.Meta%d, S.MetaAvg%d, S.MetaMax%d);\n", S.TimerInfo[i].nTimerIndex, S.TimerInfo[i].nTimerIndex, S.TimerInfo[i].pName, S.TimerInfo[i].nGroupIndex,
			MICROPROFILE_UNPACK_RED(nColor) & 0xff,
			MICROPROFILE_UNPACK_GREEN(nColor) & 0xff,
			MICROPROFILE_UNPACK_BLUE(nColor) & 0xff,
			MICROPROFILE_UNPACK_RED(nColorDark) & 0xff,
	...
	for (uint32_t i = 0; i < S.nNumLogs; ++i)
	{
		if (S.Pool[i])
		{
			MicroProfilePrintf2(CB, Handle, "'%s',", S.Pool[i]->ThreadName);
		}
		else
		{
			MicroProfilePrintf2(CB, Handle, "'Thread %d',", i);
		}
	}
	MicroProfilePrintf2(CB, Handle, "];\n\n");
	MicroProfilePrintf2(CB, Handle, "\nS.ISGPU = [");
	for (uint32_t i = 0; i < S.nNumLogs; ++i)
	{
		MicroProfilePrintf2(CB, Handle, "%d,", (S.Pool[i] && S.Pool[i]->nGpu) ? 1 : 0);
	}
	MicroProfilePrintf2(CB, Handle, "];\n\n");
	...
	for (uint32_t i = 0; i < S.nNumLogs; ++i)
	{
		if (S.Pool[i])
		{
			MicroProfilePrintf2(CB, Handle, "S.ThreadGroupTime%d = [", i);
			float fToMs = S.Pool[i]->nGpu ? fToMsGPU : fToMsCPU;
			for (uint32_t j = 0; j < MICROPROFILE_MAX_GROUPS; ++j)
			{
				MicroProfilePrintf2(CB, Handle, "%f,", S.Pool[i]->nAggregateGroupTicks[j] / nAggregateFrames * fToMs);
			}
			MicroProfilePrintf2(CB, Handle, "];\n");
		}
	}
	...
	for (uint32_t i = 0; i < S.nNumLogs; ++i)
	{
		if (S.Pool[i])
		{
			MicroProfilePrintf2(CB, Handle, "S.ThreadGroupTimeTotal%d = [", i);
			float fToMs = S.Pool[i]->nGpu ? fToMsGPU : fToMsCPU;
			for (uint32_t j = 0; j < MICROPROFILE_MAX_GROUPS; ++j)
			{
				MicroProfilePrintf2(CB, Handle, "%f,", S.Pool[i]->nAggregateGroupTicks[j] * fToMs);
			}
			MicroProfilePrintf2(CB, Handle, "];\n");
		}
	}
	...
	for (uint32_t i = 0; i < S.nNumLogs; ++i)
	{
		if (S.Pool[i])
		{
			...
			MicroProfilePrintf2(CB, Handle, "%d,", ThreadId);
		}
		else
		{
			MicroProfilePrintf2(CB, Handle, "-1,", i);
		}
	}
	MicroProfilePrintf2(CB, Handle, "];\n\n");
	...
	for (int i = 0; i < (int)S.nNumCounters; ++i)
	{
		if (0 != (S.CounterInfo[i].nFlags & MICROPROFILE_COUNTER_FLAG_DETAILED))
		{
			...
			for (uint32_t j = 0; j < MICROPROFILE_GRAPH_HISTORY; ++j)
			{
				uint32_t nHistoryIndex = (nBaseIndex + j) % MICROPROFILE_GRAPH_HISTORY;
				int64_t nValue = MicroProfileClamp(S.nCounterHistory[nHistoryIndex][i], nCounterMin, nCounterMax);
				MicroProfilePrintf2(CB, Handle, "%lld,", nValue);
			}
			MicroProfilePrintf2(CB, Handle, "];\n");
			...
			for (uint32_t j = 0; j < MICROPROFILE_GRAPH_HISTORY; ++j)
			{
				...
				int64_t nValue = MicroProfileClamp(S.nCounterHistory[nHistoryIndex][i], nCounterMin, nCounterMax);
				float fPrc = (nValue + nCounterOffset) * fRcp;
				MicroProfilePrintf2(CB, Handle, "%f,", fPrc);
			}
			MicroProfilePrintf2(CB, Handle, "];\n");
			MicroProfilePrintf2(CB, Handle, "S.CounterHistory%d = MakeCounterHistory(%d, S.CounterHistoryArray%d, S.CounterHistoryArrayPrc%d)\n", i, i, i, i);
		}
		else
		{
			MicroProfilePrintf2(CB, Handle, "S.CounterHistory%d;\n", i);
		}
	}
	...
	for (int i = 0; i < (int)S.nNumCounters; ++i)
	{
		...
		MicroProfileFormatCounter(S.CounterInfo[i].eFormat, nCounter, Formatted, sizeof(Formatted) - 1);
		MicroProfileFormatCounter(S.CounterInfo[i].eFormat, S.CounterInfo[i].nLimit, FormattedLimit, sizeof(FormattedLimit) - 1);
		MicroProfilePrintf2(CB, Handle, "MakeCounter(%d, %d, %d, %d, %d, '%s', %lld, %lld, %lld, '%s', %lld, '%s', %f, %f, %d, S.CounterHistory%d),",
			i,
			S.CounterInfo[i].nParent,
			S.CounterInfo[i].nSibling,
			S.CounterInfo[i].nFirstChild,
	...
	{
		MicroProfilePrintf2(CB, Handle, " //Timeline begin\n");
		MicroProfileThreadLog* pLog = &S.TimelineLog;
		uint32_t nFrameIndexFirst = (nFirstFrame) % MICROPROFILE_MAX_FRAME_HISTORY;
		uint32_t nFrameIndexLast = (nFirstFrame + nNumFrames) % MICROPROFILE_MAX_FRAME_HISTORY;
		{
...
#define pp(...) MicroProfilePrintf2(CB, Handle, __VA_ARGS__)
		...
		MicroProfilePrintf2(CB, Handle, " //Timeline end\n");
	}

	MicroProfilePrintf2(CB, Handle, "S.Frames = Array(%d);\n", nNumFrames);
	for (uint32_t i = 0; i < nNumFrames; ++i)
	{
		uint32_t nFrameIndex = (nFirstFrame + i) % MICROPROFILE_MAX_FRAME_HISTORY;
		uint32_t nFrameIndexNext = (nFrameIndex + 1) % MICROPROFILE_MAX_FRAME_HISTORY;
		...
		for (uint32_t j = 0; j < S.nNumLogs; ++j)
		{
			...
			float fToMsCpu = MicroProfileTickToMsMultiplier(nTicksPerSecondCpu);
			float fToMsBase = MicroProfileTickToMsMultiplier(pLog->nGpu ? nTicksPerSecondGpu : nTicksPerSecondCpu);
			MicroProfilePrintf2(CB, Handle, "S.ts_%d_%d = [", i, j);
			if (nLogStart != nLogEnd)
			{
				int f = 0;
				for (uint32_t k = nLogStart; k != nLogEnd; k = (k + 1) % MICROPROFILE_BUFFER_SIZE)
				{
					...
					MicroProfilePrintf2(CB, Handle, f++ ? ",%f" : "%f", fTime);
				}
			}
			MicroProfilePrintf2(CB, Handle, "];\n");
			MicroProfilePrintf2(CB, Handle, "S.tt_%d_%d = [", i, j);
			if (nLogStart != nLogEnd)
			{
				uint32_t k = nLogStart;
				MicroProfilePrintf2(CB, Handle, "%d", MicroProfileLogGetType(pLog->Log[k]));
				for (k = (k + 1) % MICROPROFILE_BUFFER_SIZE; k != nLogEnd; k = (k + 1) % MICROPROFILE_BUFFER_SIZE)
				{
					uint64_t nLogType2 = MicroProfileLogGetType(pLog->Log[k]);
					MicroProfilePrintf2(CB, Handle, ",%d", nLogType2);
				}
			}
			MicroProfilePrintf2(CB, Handle, "];\n");

			MicroProfilePrintf2(CB, Handle, "S.ti_%d_%d = [", i, j);
			if (nLogStart != nLogEnd)
			{
				for (uint32_t k = nLogStart; k != nLogEnd; k = (k + 1) % MICROPROFILE_BUFFER_SIZE)
				{
					...
					if (nLogType == MP_LOG_ENTER || nLogType == MP_LOG_LEAVE)
					{
						...
						MicroProfilePrintf2(CB, Handle, pFormat, nTimerIndex);
					}
					else
					{
						MicroProfilePrintf2(CB, Handle, pFormat, -1);
					}
				}
			}
			MicroProfilePrintf2(CB, Handle, "];\n");
		...
		MicroProfilePrintf2(CB, Handle, "S.ts%d = [", i);
		for (uint32_t j = 0; j < S.nNumLogs; ++j)
		{
			MicroProfilePrintf2(CB, Handle, "S.ts_%d_%d,", i, j);
		}
		MicroProfilePrintf2(CB, Handle, "];\n");
		MicroProfilePrintf2(CB, Handle, "S.tt%d = [", i);
		for (uint32_t j = 0; j < S.nNumLogs; ++j)
		{
			MicroProfilePrintf2(CB, Handle, "S.tt_%d_%d,", i, j);
		}
		...
		MicroProfilePrintf2(CB, Handle, "S.ti%d = [", i);
		for (uint32_t j = 0; j < S.nNumLogs; ++j)
		{
			MicroProfilePrintf2(CB, Handle, "S.ti_%d_%d,", i, j);
		}
		...
		MicroProfilePrintf2(CB, Handle, "S.Frames[%d] = MakeFrame(%d, %f, %f, %f, %f, S.ts%d, S.tt%d, S.ti%d);\n", i, 0, fFrameMs, fFrameEndMs, fFrameGpuMs, fFrameGpuEndMs, i, i, i);
	}
	...
	uint32_t nWrittenBefore = S.nWebServerDataSent;
	MicroProfilePrintf2(CB, Handle, "S.CSwitchThreadInOutCpu = [");
	for (uint32_t j = nContextSwitchStart; j != nContextSwitchEnd; j = (j + 1) % MICROPROFILE_CONTEXT_SWITCH_BUFFER_SIZE)
	{
		MicroProfileContextSwitch CS = S.ContextSwitch[j];
		int nCpu = CS.nCpu;
		MicroProfilePrintf2(CB, Handle, "%d,%d,%d,", CS.nThreadIn, CS.nThreadOut, nCpu);
	}
	MicroProfilePrintf2(CB, Handle, "];\n");
	MicroProfilePrintf2(CB, Handle, "S.CSwitchTime = [");
	float fToMsCpu = MicroProfileTickToMsMultiplier(MicroProfileTicksPerSecondCpu());
	for (uint32_t j = nContextSwitchStart; j != nContextSwitchEnd; j = (j + 1) % MICROPROFILE_CONTEXT_SWITCH_BUFFER_SIZE)
	{
		MicroProfileContextSwitch CS = S.ContextSwitch[j];
		float fTime = MicroProfileLogTickDifference(nTickStart, CS.nTicks) * fToMsCpu;
		MicroProfilePrintf2(CB, Handle, "%f,", fTime);
	}
	MicroProfilePrintf2(CB, Handle, "];\n");

	MicroProfilePrintf2(CB, Handle, "S.CSwitchThreads = {");
	...
	for (uint32_t i = 0; i < nNumThreads; ++i)
	{
		...
		MicroProfilePrintf2(CB, Handle, "%" PRId64 ":{\'tid\':%" PRId64 ",\'pid\':%" PRId64 ",\'t\':\'%s\',\'p\':\'%s\'},",
			(uint64_t)pThreadInfo[i].tid,
			(uint64_t)pThreadInfo[i].tid,
			(uint64_t)pThreadInfo[i].pid,
			p1, p2
	...
	uint32_t nWrittenAfter = S.nWebServerDataSent;
	MicroProfilePrintf2(CB, Handle, "//CSwitch Size %d\n", nWrittenAfter - nWrittenBefore);
	...
	{
		uint32_t idx = nGroupCounterSort[i];
		MicroProfilePrintf2(CB, Handle, "%8d:%s\n", nGroupCounter[idx], S.GroupInfo[idx].pName);
	}
	MicroProfilePrintf2(CB, Handle, "Marker Per Timer\n");
	for (uint32_t i = 0; i < S.nTotalTimers; ++i)
	{
		uint32_t idx = nTimerCounterSort[i];
		MicroProfilePrintf2(CB, Handle, "%8d:%s(%s)\n", nTimerCounter[idx], S.TimerInfo[idx].pName, S.GroupInfo[S.TimerInfo[idx].nGroupIndex].pName);
	}
	MicroProfilePrintf2(CB, Handle, "\n-->\n");
...
{
    return (S.bContextSwitchStop || !S.bContextSwitchRunning) ? 0 : 1;
}
...
void MicroProfileWin32InitThreadInfo2()
{
    ...
    if (Thread32First(hSnap, &te32))
    {
        ...
        do
        {
            nThreadsTested++;
            const char* pModule = "?";
            HANDLE hThread = OpenThread(THREAD_QUERY_INFORMATION, 0, te32.th32ThreadID);
            if (hThread)
            {
...
void MicroProfileInstrumentWithoutSymbols2(const char* pModules, const char* pSymbols)
{
    char SymString[512];
    const char* pStr = 0;
    if (MicroProfileSymInit())
    {
        HANDLE h = GetCurrentProcess();        
        int nCount = snprintf(SymString, sizeof(SymString) - 1, "%s!%s", pModules, pSymbols);
        if (nCount <= sizeof(SymString) - 1)
        {
            g_pFunctionFoundHack = 0;
            if (SymEnumSymbols(h, 0, SymString, MicroProfileQueryContextEnumSymbols1, 0))
            {
                if (g_pFunctionFoundHack)
                {
                    uint32_t nColor = MicroProfileColorFromString(pSymbols);
                    const char* pDemangled = pSymbols;//MicroProfileDemangleSymbol(pSymbols[i]);
                    MicroProfileInstrumentFunction(g_pFunctionFoundHack, pModules, pDemangled, nColor);
                }
            }
        }        
        MicroProfileSymCleanup();
...
void MicroProfileJusticeFormatNowTime(char* pFormattedTime)
{
    ...
    strftime(pFormattedTime, 50, "%Y-%m-%dT%H:%M:%S%z", timeinfo);
}
...
void MicroProfileJusticeEventsStart()
{
    MicroProfileJusticeBufferReset();

    S.bIsJusticeEventsSending = true;
    MicroProfileThreadStart(&S.JusticeEventsThread, MicroProfileJusticeSenderThread);
}

void MicroProfileJusticeEventsStop()
{
    S.bIsJusticeEventsSending = false;
    MicroProfileThreadJoin(&S.JusticeEventsThread);
}

void MicroProfileJusticeBufferReset()
{
    S.JusticeBuffer.nGet.store(0);
    S.JusticeBuffer.nPut.store(0);
}

void MicroProfileJusticeEventsUpdate()
{
    if (S.bIsJusticeEventsSending)
    {
        float fTickToMsCpu = MicroProfileTickToMsMultiplier(MicroProfileTicksPerSecondCpu());
        float fTickToMsGpu = MicroProfileTickToMsMultiplier(MicroProfileTicksPerSecondGpu());
        MicroProfileFrameState* pFrameCurrent = &S.Frames[S.nFrameCurrent];
        ...
        while (-1 != nTimer)
        {
            MicroProfileTimerInfo& TI = S.TimerInfo[nTimer];
            float fTickToMs = TI.Type == MicroProfileTokenTypeGpu ? fTickToMsGpu : fTickToMsCpu;
            fTimeDI = fTickToMs * S.Frame[nTimer].nTicks;
            nTimer = TI.nWSNext;
        }
        //------------------------------------------------------------------

        MicroProfileJusticeBufferPut(fTime, fTimeDI);

        // send dynamic instrumentation
    }
}

bool MicroProfileJusticeBufferPut(float item, float difunc)
{
    uint32_t nGet = S.JusticeBuffer.nGet.load();
    uint32_t nPut = S.JusticeBuffer.nPut.load();
	uint32_t nextPut = (nPut + 1) % MICROPROFILE_EVENT_BUFFER_SIZE;

    if (nextPut == nGet) //is full
    {
        return false;
    }

    S.JusticeBuffer.Data[nPut] = item;
    S.JusticeBuffer.DynamicFunc[0][nPut] = difunc;

    S.JusticeBuffer.nPut.store(nextPut);

    return true;
}

bool MicroProfileJusticeBufferGet(float* pItem, float* pItemDI)
{
    uint32_t nGet = S.JusticeBuffer.nGet.load();
    uint32_t nPut = S.JusticeBuffer.nPut.load();

    if (nGet == nPut) // is empty
    {
        return false;
    }

    *pItem = S.JusticeBuffer.Data[nGet];
    *pItemDI = S.JusticeBuffer.DynamicFunc[0][nGet];
    nGet = (nGet + 1) % MICROPROFILE_EVENT_BUFFER_SIZE;

    S.JusticeBuffer.nGet.store(nGet);
...
{
#if 0
    while (S.bIsJusticeEventsSending)
    {
        MicroProfileSleep(9000);

#ifdef _WIN32
        WSADATA wsa;
        if (WSAStartup(MAKEWORD(2, 2), &wsa))
        {
            S.JusticeEventsSocket = (MpSocket)-1;
            return nullptr;
        }
#endif

        S.JusticeEventsSocket = socket(AF_INET, SOCK_STREAM, IPPROTO_TCP);
        MP_ASSERT(!MP_INVALID_SOCKET(S.JusticeEventsSocket));

        {
            int r = 0;
            int on = 1;
#if defined(_WIN32)
            r = setsockopt(S.JusticeEventsSocket, SOL_SOCKET, SO_REUSEADDR, (const char*)&on, sizeof(on));
#else
            r = setsockopt(S.JusticeEventsSocket, SOL_SOCKET, SO_REUSEADDR, (void*)&on, sizeof(on));
#endif
        }
        ...
        if (int error = getaddrinfo("analytics.accelbyte.net", "80", &hints, &res) != 0)
        {
            //keep trying to resolve analytics.accelbyte.net
#if 1
            MicroprofileDebugLog("getaddrinfo error\r\n");
#endif
            continue;
        }


        if (connect(S.JusticeEventsSocket, res->ai_addr, res->ai_addrlen) < 0)
        {
            //keep trying to contact analytics.accelbyte.net
#if 1
            MicroprofileDebugLog("connect error\r\n");
#endif
            continue;
        }
        ...
        char strUuid[17];
        char timeStamp[25];
        MicroProfileJusticeFormatNowTime(timeStamp);
        MicroProfileJusticeCreateUuid(strUuid);

        char msgBody[20480];
        float fTime;
        int nBodyLength = 0;
        nBodyLength += snprintf(msgBody + nBodyLength, sizeof(msgBody) - nBodyLength, "{\"Data\": {\"values\": [");

        while (MicroProfileJusticeBufferGet(&fTime))
        {
            nBodyLength += snprintf(msgBody + nBodyLength, sizeof(msgBody) - nBodyLength, "%.2f,", fTime);
        }

        --nBodyLength;//remove comma
        nBodyLength += snprintf(msgBody + nBodyLength, sizeof(msgBody) - nBodyLength,
            "]},"
            "\"AgentType\": 0,"
            "\"ClientID\": \"%s\","
            "\"Time\": \"%s\","
            ...
            MICROPROFILE_JUSTICE_CLIENTID,
            timeStamp,
            "dbe1913f193e444ab569dafab7854903");

        char msgHeader[1024];
        const char* headerFormat =
            "POST /telemetry/public/namespaces/%s/events/314/60/1/4001 HTTP/1.1\r\n"
            "Host: analytics.accelbyte.net\r\n"
            "Content-Type: application/json\r\n"
            "Content-Length: %d\r\n"
            "\r\n";

        int nHeaderLength = snprintf(msgHeader, sizeof(msgHeader), headerFormat,
            MICROPROFILE_JUSTICE_NAMESPACE,
            nBodyLength);

        int nSent = send(S.JusticeEventsSocket, msgHeader, nHeaderLength, 0);

        if (nSent < 0)
        {
            MicroprofileDebugLog("cannot send header\r\n");
        }

        nSent = send(S.JusticeEventsSocket, msgBody, nBodyLength, 0);

        if (nSent < 0)
        {
            MicroprofileDebugLog("cannot send body\r\n");
        }

        char msgReceived[4096];

        int nRecvd = recv(S.JusticeEventsSocket, msgReceived, sizeof(msgReceived), 0);

        if (nRecvd < 0)
        {
            MicroprofileDebugLog("cannot receive\r\n");
        }

#ifdef _WIN32
        closesocket(S.JusticeEventsSocket);
        WSACleanup();
#else
        close(S.JusticeEventsSocket);
#endif
    }
#endif
    return nullptr;
